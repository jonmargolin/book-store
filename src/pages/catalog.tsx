...
const Catalog = () => {
  ...
  const updateSelectedBookItem = (id: string) => { 
    contextDispatch({ type: 'UPDATE_SELECTED_ID',payload: {selectedId:id}})
    setModalOpen(true)
} 
 const handelPagniationChnage =(pagination:{value:number, label:string}[]) => {
   const maxResults = pagination[0].value
  contextDispatch({ type:"UPDATE_PAGINATION", payload: {maxResults}})
  mutateBooks({...params, maxResults},{onSuccess:(data)=> contextDispatch({type: 'UPDATE_DATA', payload:data.data}),onError: (err) => handelError(err.message)})
 }
  const handePageChange =(direction: number) =>{
     ...
     contextDispatch({type:"UPDATE_PAGE_POSITION", payload: {startIndex:prevPage}})
     mutateBooks({...params, ...{startIndex: prevPage}},{onSuccess:(data)=> contextDispatch({type: 'UPDATE_DATA', payload:data.data}),onError: (err) => handelError(err.message)})
    }
    if(nextPage< totalItems && direction ===1){
      contextDispatch({type:"UPDATE_PAGE_POSITION", payload: {startIndex:nextPage}})
      mutateBooks({...params,  ...{startIndex: nextPage}},{onSuccess:(data)=> contextDispatch({type: 'UPDATE_DATA', payload:data.data}),onError: (err) => handelError(err.message)})
    }
  }
  ...
  useEffect(() => {
    try {
      mutateBooks({...params},{onSuccess:(data)=> contextDispatch({type: 'UPDATE_DATA', payload:data.data}), onError: (err) => handelError(err.message)})
    }
    catch(error){
      console.log(error)
     handelError()
  ...
  if(isLoading){
    ...
    return (
      ...
      <CatalogHeader/>
          ...
          <SearchBox value={searchTerm} onChange={handleChange} onClick={()=> cleartSerachTerm()} />
    ...
    <Modal
        ...
        ariaHideApp={false}
      >
        <BookModalBody closeModal={() => setModalOpen(false)} addBook={ () => addBookToCart()} />
        </Modal>
        <ToastContainer />
      </main>
